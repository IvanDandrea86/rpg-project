...
let damageCalculation = (attacker, defender) => {
    ...
    if (attacker.lifeSteal != 0) {
        ...
        writeOnConsole(`${attacker.heroName} steal ${Number(lifesteal.toFixed(1))} lifepoints\n`, console_action);
    ...
    if (abilitychance(defender.dogeChance) == true) { //double attack calculator
        ...
        if (defender.preventDmg > 0) {
            ...
            writeOnConsole(` ${defender.heroName} has now ${Number(defender.currenthealth).toFixed(1)}\n`, console_action)
        }
        else {
            defender.currenthealth = defender.currenthealth - totdmg
            writeOnConsole(`${attacker.heroName} successfully attacked and deals ${totdmg}.\n
...
/**
 *  Check Healt of Defender and Attackr
 * @param {Person} attacker 
 * @param {Person} defender 
 */
...
let drawStat = (htmlElem, person) => {
    const attributeList = ["extraDmg", "doubleAttack", "bonusHealth", "healingPower", "dogeChance", "preventDmg", "lifeSteal", "deflectDmg"]
